...
from pointnet import PointNetCls, PointGen, PointGenC, PointNetAE, PointNetfeat
...
parser.add_argument('--outf', type=str, default='ae3',  help='output folder')
...
class OffsetNet(nn.Module):
    def __init__(self, num_points = 2048):
        ...
        self.net = nn.Sequential(
                    PointNetfeat(num_points = self.num_points, trans = False),
                    nn.Linear(1024, 64),
                    nn.ReLU(),
                    nn.Linear(64,32),
                    nn.ReLU()
    ...
    def forward(self, x):
        ...
        for i in range(4):
            ...
            #y.append(self.nets[i](x[:,i,:,:]).view(batchsize, 1, 3))
            y.append(self.net(x[:,i,:,:]).view(batchsize, 1, 32))

        y = torch.cat(y, 2)
        y = y.view(batchsize, -1)
        y = self.fcs(y)
        y = y.view(batchsize, 4, 3)
        return y
...
optimizerG = optim.Adagrad(list(gen.parameters()) + list( off.parameters()), lr = 0.001)
...
for epoch in range(opt.nepoch):
    ...
    torch.save(off.state_dict(), '%s/modeloff_%d.pth' % (opt.outf, epoch))
