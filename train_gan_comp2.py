...
from pointnet import PointNetCls, PointGen, PointGenC, PointGenComp2
...
parser.add_argument('--outf', type=str, default='gan_comp',  help='output folder')
...
for epoch in range(opt.nepoch):
    for i, data in enumerate(dataloader, 0):
        ...
        sim_noise = Variable(torch.randn(bs, 1024)).cuda()
        points_gen = gen(incomp, sim_noise)
        
        pred, trans = classifier(points_gen)
        target = Variable(torch.from_numpy(np.ones(bs,).astype(np.int64))).cuda()
        #print(pred, target)
        dist1, dist2 = dist(incomp.transpose(2,1), points_gen.transpose(2,1))
        
        lossG2 = (torch.mean(dist1)) * 100
        ...
        print('[%d: %d/%d] train lossD: %f lossG: %f lossG2 %f' %(epoch, i, num_batch, lossD.data[0], lossG1.data[0], lossG2.data[0]))
